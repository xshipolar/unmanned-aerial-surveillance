...
void FerNNClassifier::read(const FileNode& file){
  ...
  _valid = (float)file["valid"];
  _ncc_thesame = (float)file["ncc_thesame"];
  _nstructs = (int)file["num_trees"];
  _struct_size = (int)file["num_features"];
  _thr_fern = (float)file["thr_fern"];
  _thr_nn = (float)file["thr_nn"];
...
void FerNNClassifier::prepare(const vector<Size>& scales){
    ...
    int totalFeatures = _nstructs*_struct_size;
    ...
    for (int i = 0; i<_nstructs; i++) {
        posteriors.push_back(vector<float>(pow(2.0,_struct_size), 0));
        pCounter.push_back(vector<int>(pow(2.0,_struct_size), 0));
        nCounter.push_back(vector<int>(pow(2.0,_struct_size), 0));
    }
}

void FerNNClassifier::getFeatures(const cv::Mat& image,const int& scale_idx, vector<int>& fern){
    int leaf;
    for (int t=0;t<_nstructs;t++){
        leaf=0;
        for (int f=0; f<_struct_size; f++){
            leaf = (leaf << 1) + features[scale_idx][t*_nstructs+f](image);
        }
        fern[t]=leaf;
    }
}
...
void FerNNClassifier::trainF(const vector<std::pair<vector<int>,int> >& ferns,int resample){
    ...
    //  double thrP   = *mxGetPr(prhs[3]) * nTREES; ->threshold*_nstructs
    //  int bootstrap = (int) *mxGetPr(prhs[4]); ->resample
    thrP = _thr_fern*_nstructs;                                                          // int step = numX / 10;
...
void FerNNClassifier::trainNN(const vector<cv::Mat>& nn_examples){
    ...
    for (int i=0;i<nn_examples.size();i++){                          //  For each example
        ...
        if (y[i]==1 && conf<=_thr_nn){                               //    if y(i) == 1 && conf1 <= tld.model.thr_nn % 0.65
...
void FerNNClassifier::evaluateTh(const vector<pair<vector<int>,int> >& nXT,const vector<cv::Mat>& nExT){
  float fconf;
    for (int i=0;i<nXT.size();i++){
      fconf = (float) measure_forest(nXT[i].first)/_nstructs;
      if (fconf>_thr_fern)
        _thr_fern=fconf;
  }
    vector <int> isin;
    ...
    if (_thr_nn>thr_nn_valid)
      thr_nn_valid = _thr_nn;
}
