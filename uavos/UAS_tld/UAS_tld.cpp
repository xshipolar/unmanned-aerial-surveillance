...
void UAS_tld::init(){
    ...
    parameter_file.open("../parameters.yml", FileStorage::READ);
    ...
    if (!capture_device.isOpened()) {
        ...
        capture_device.set(CV_CAP_PROP_FRAME_WIDTH,340);
        capture_device.set(CV_CAP_PROP_FRAME_HEIGHT,240);

        // Set TLD with stored paramters
        _tld_vision.read(parameter_file.getFirstTopLevelNode());
...
void UAS_tld::readyTracking(const Rect& input_box){
    ...
    capture_device >> _current_color_frame;
...
void UAS_tld::updateTracking(){
    if (capture_device.read(_current_color_frame)){
        ...
        _tld_vision.processFrame(_previous_gray_frame, _current_gray_frame, 
                                 ...
                                 target_box, _track_status, true); // TODO: Track and Learn is always on for now
