...
height=2; % Average height in miles*10
Xv0=5; % Initial X position of the vehicle
...
for k=1:tf
    ...
    Phi(k)=pi/2-atan(Rho(k)/height);
    [x(k),y(k),z(k)]=sph2cart(pi/2-(Psi(k)+Psih(k)),-Phi(k),1); %drawn gimbal vectors for the final plot
end


 figure()
 plot3(Xv,Yv,Zv,Xg,Yg,Zg);
 hold on;
 plot(Xv(1:1000:tf+1),Yv(1:1000:tf+1),'bo');
 hold on;
 quiver3(Xg(2:1000:tf+1),Yg(2:1000:tf+1),Zg(2:1000:tf+1),x(2:1000:tf+1),y(2:1000:tf+1),z(2:1000:tf+1),0.2,'r');
